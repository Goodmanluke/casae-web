...
export default function CMA() {
  ...
  const [conditionMode, setConditionMode] =
    useState<ConditionInputMode>("simple");
  ...
  const runBaselineCMA = async () => {
    ...
    if (!subscription && !isPremium && !isPro) {
      setError(
        "You need an active Premium or Pro subscription to use the CMA feature."
      );
      return;
    }
          ...
          !error && (
            <div className="rounded-xl bg-white/10 p-6 border border-white/10 text-center mt-6">
              ...
              <div className="opacity-80">
                Follow the step-by-step process to get comprehensive property
